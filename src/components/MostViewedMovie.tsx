...
export default function MostViewedMovie({ item, onWatchNow, onOpenDetails }: MostViewedMovieProps) {
  const bg = item.backdropOriginalUrl || item.backdropUrl;

  return (
    <div className="w-full px-2 lg:px-3">
      <div className="relative w-full overflow-hidden sm:rounded-2xl border border-white/10 shadow-2xl shadow-black/60">
        <div className="relative aspect-[4/3] sm:aspect-[21/8] w-full">
          {bg ? (
            <Image
              src={bg}
              alt={item.title}

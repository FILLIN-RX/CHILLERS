...
import { ArrowLeft, Play, Star, CalendarBlank, FilmSlate, ShareNetwork, BookmarkSimple, X, Check, Sparkle, LinkSimple, ListNumbers } from "@phosphor-icons/react";

interface TvClientWrapperProps {
  id: string;
  initialItem: MovieOrShow | null;
  initialSimilar: MovieOrShow[];
}

export default function TvClientWrapper({ id, initialItem, initialSimilar }: TvClientWrapperProps) {
  ...
  useEffect(() => {
    ...
    const checkDispo = async () => {
      ...
      if (!dispo?.disponible && item.title) {
        ...
        const pollTimer = setInterval(async () => {
          ...
          const updatedDispo = await getDisponible(id, "series", item.title).catch(() => null);
      ...
      <div className="w-full px-4 sm:px-8 md:px-12 lg:px-16 py-10 space-y-12">
        ...
        {item.castDetails && item.castDetails.length > 0 && (
          <section className="space-y-4 pt-6 border-t border-zinc-800/80">
            <div className="flex items-center justify-between">
              <h2 className="text-lg sm:text-2xl font-black text-white">Casting & Personnages</h2>
              <span className="text-xs text-zinc-500 font-medium">{item.castDetails.length} acteurs</span>
            </div>
      ...
      {trailerOpen && item.trailerUrl && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="relative w-full max-w-4xl aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl border border-zinc-800">
            <button onClick={() => setTrailerOpen(false)} className="absolute top-4 right-4 z-20 p-2 rounded-full bg-black/60 text-white hover:bg-black/90 transition-colors">
              <X className="w-6 h-6" />
            </button>
            <iframe

...
async function searchOtaku(title, type = 'movie') {
    try {
        ...
        const { data } = await axios_1.default.get(`${BASE_URL}/api/fs-search`, {
            params: { q: title },
            timeout: 15000,
            headers: { 'User-Agent': 'Mozilla/5.0' }
        });
        ...
        const { data: watch } = await axios_1.default.get(`${BASE_URL}/api/fs-watch`, {
...
async function getSpecificEpisodeLink(page, episodeNumber, previousLink, seriesIdOrTitle) {
    try {

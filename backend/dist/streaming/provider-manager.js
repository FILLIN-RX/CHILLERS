...
class ProviderManager {
    ...
    buildProviders() {
        return [
            ...
            new torrserver_provider_1.TorrServerProvider(),
        ];
    }
    async getMovieStream(query, opts) {
        // ── Cache LRU ───────────────────────────────────────────────────────────
    ...
    async getEpisodeStream(query, opts) {
        // ── Cache LRU ───────────────────────────────────────────────────────────
    ...
    async tryProvider(provider, type, query) {
        ...
        const timeout = provider.name === 'otaku'
            ? OTAKU_TIMEOUT
        ...
        try {
            ...
            const valid = provider.name === 'mongodb' ||
                provider.name === 'torrserver' ||
    ...
    async filterProviders(query, only) {

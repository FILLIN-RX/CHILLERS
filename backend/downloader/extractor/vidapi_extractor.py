from downloader.config import VIDAPI_BASE, EXTRACTION_TIMEOUT
...
class VidAPIExtractor(BaseExtractor):
    """Extracteur fallback via VidAPI (https://vidapi.xyz)."""
    ...
    async def extract_m3u8(
        ...
        if media_type == "movie":
            url = f"{VIDAPI_BASE}/movie/{media_id}"
        else:
            season = season or 1
            episode = episode or 1
            url = f"{VIDAPI_BASE}/tv/{media_id}/{season}/{episode}"

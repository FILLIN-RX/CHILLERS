...
import { TorrServerProvider } from './providers/torrserver.provider';
...
export class ProviderManager {
  ...
  async getMovieStream(query: StreamQuery, opts?: { only?: string[] }): Promise<CachedStream | null> {
    // ── Cache LRU ───────────────────────────────────────────────────────────
    ...
    const activeProviders = await this.filterProviders(query, opts?.only);
  ...
  async getEpisodeStream(query: StreamQuery, opts?: { only?: string[] }): Promise<CachedStream | null> {
    // ── Cache LRU ───────────────────────────────────────────────────────────
  ...
  private async tryProvider(
    ...
    try {
      ...
      // Skip validation for MongoDB and TorrServer — URLs are generated by
      // our own services (no external liveness check needed; a HEAD on the
      // transcode endpoint would spawn a full FFmpeg process).
      const valid =
        provider.name === 'mongodb' ||
        provider.name === 'torrserver' ||
        (await this.validateUrl(result.embedUrl));
  ...
  private sortProviders(query: StreamQuery, only?: string[]): StreamingProvider[] {
    const base = only ? this.providers.filter((p) => only.includes(p.name)) : this.providers;
  ...
  private async filterProviders(query: StreamQuery, only?: string[]): Promise<StreamingProvider[]> {
    return this.sortProviders(query, only);
  }
